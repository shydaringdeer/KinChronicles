...
const pathfindingAStarDiagonal = (grid, start, end) => {
  try {
    ...
    const smoothedPath = pathfinding.Util.smoothenPath(grid, fullPath);
...
const pathfindingJumpPointNoDiagonal = (grid, start, end) => {
  try {
    // FIXME: The "pathfinding" module doe not have proper typings.
    // @ts-ignore
    const finder = new pathfinding.JumpPointFinder({
      diagonalMovement: pathfinding.DiagonalMovement.Never
    });
    const fullPath = finder.findPath(start.x, start.y, end.x, end.y, grid);
...
function SmartEdge(_ref) {
  ...
  const FallbackEdge = options.fallback || react.BezierEdge;
  ...
  return /*#__PURE__*/React.createElement(react.BaseEdge, {
    path: svgPathString,
    labelX: edgeCenterX,
    labelY: edgeCenterY,
    label: label,
...
const BezierConfiguration = {
  drawEdge: svgDrawSmoothLinePath,
  generatePath: pathfindingAStarDiagonal,
  fallback: react.BezierEdge // eslint-disable-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
};
function SmartBezierEdge(props) {
  const nodes = react.useNodes();
...
const StepConfiguration = {
  ...
  fallback: react.StepEdge // eslint-disable-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
};
...
const StraightConfiguration = {
  ...
  fallback: react.StraightEdge // eslint-disable-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
};
